...
from model import *
from model.potential.hdf_net import HydrationDistributionFunctionNet


HDF_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "../data/hdf")


class HydrationPotentialCylinderNet:
    def __init__(
        self,
        r0,
        ion_type,
    ...
    def evaluate(self):
        ...
        factor = CUPY_FLOAT(self._h**3 * self._n_bulk)
        pore_file_path = os.path.join(self._hdf_dir, "pore-water-%s.pkl" % PRECISION)
        ion_file_path = os.path.join(
            self._hdf_dir, "%s-water-%s.pkl" % (self._ion_name, PRECISION)
        )
        g_pore = HydrationDistributionFunctionNet(net_file_path=pore_file_path)
        g_ion = HydrationDistributionFunctionNet(net_file_path=ion_file_path)
        f = g_pore(dist_pore1) * g_pore(dist_pore2)
        g = g_ion(dist_ion)
        threshold = CUPY_FLOAT(1e-5)
...
if __name__ == "__main__":
    ...
    # r0_list = [i * cc_bond_length * 3 / (2 * np.pi) for i in range(12, 15)]
