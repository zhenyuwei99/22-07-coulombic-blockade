...
def generate_json_file_path(out_dir, r0, w0, l0, ls, ions, electric_field):
    ion_name = "-".join(
        [
            "%s-%.2emolPerL"
...
if __name__ == "__main__":
    ...
    r0_list = [Quantity(5, angstrom), Quantity(10, angstrom), Quantity(20, angstrom)]
    ...
    electric_field_list = [0.001, 0.01, 0.1, 0.5, 1]
    ...
    job_args = list(
        product(r0_list, l0_list, w0_list, ls_list, ions_list, electric_field_list)
    )
    print("%s jobs in total" % (len(job_args)))
    json_file_path_list = []
    for r0, l0, w0, ls, ions, electric_field in job_args:
