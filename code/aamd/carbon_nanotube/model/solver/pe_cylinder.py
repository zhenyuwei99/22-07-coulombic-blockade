...
class PECylinderSolver:
    ...
    def _get_coefficient(self):
        ...
        # r
        factor[0, :, :-1] = pre_factor * (
            self._grid.field.epsilon[1:, 1:-1] + self._grid.field.epsilon[:-1, 1:-1]
        )
        inv_denominator += factor[0, 1:, :-1] + factor[0, :-1, :-1]
        # z
        factor[1, :-1, :] = pre_factor * (
            self._grid.field.epsilon[1:-1, 1:] + self._grid.field.epsilon[1:-1, :-1]
        )
        inv_denominator += factor[1, :-1, 1:] + factor[1, :-1, :-1]
    ...
    def _update_inner_point(self, phi, factor, inv_denominator, scaled_rho, soa_factor):
        ...
        # r
        nominator += factor[0, 1:, :-1] * phi.value[2:, 1:-1]
        nominator += factor[0, :-1, :-1] * phi.value[:-2, 1:-1]
        # z
        nominator += factor[1, :-1, 1:] * phi.value[1:-1, 2:]
        nominator += factor[1, :-1, :-1] * phi.value[1:-1, :-2]
        # Add charge
