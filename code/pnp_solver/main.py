...
def job(voltage, sod_density, cla_density):
    ...
    alpha = 2
    nanopore_shape = 1 / (
        (1 + cp.exp(-alpha * (r - extend_radius)))
    ...
    file_name = (
        "pnp-minus-%.3f-volt.grid" if voltage <= 0 else "pnp-plus-%.3f-volt.grid"
    )
    writer = GridWriter(os.path.join(out_dir, file_name % abs(voltage)))
...
if __name__ == "__main__":
    ...
    out_dir = os.path.join(cur_dir, "out/5nm-pre-fixed-2")
    os.system("rm -rf %s/*.png" % cur_dir)
    # Hyper parameter
    for v in np.linspace(-2, 2, 20, endpoint=True):
        voltage = (
            Quantity(v, volt)
        ...
        job(voltage=voltage, sod_density=sod_density, cla_density=cla_density)
